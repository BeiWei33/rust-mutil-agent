...
当前进度：`src-tauri/src/workspace/patch.rs` 已落地补丁提案模型、统一 diff 生成、SQLite 持久化、`workspace.applyPatch` 审批请求创建、已审批补丁手动应用、应用后推荐验证命令自动运行，以及关联任务的 `patchApplied` / `patchVerification` artifact 与 `artifactCreated` 事件写回；前端项目页、审批页和任务看板已能展示对应状态。回滚、验证失败返工和任务调度联动仍未实现。
...
当前状态：补丁提案存储、单文件 diff 预览、审批请求链路、审批后手动应用、应用后推荐验证命令自动运行和任务 artifact 写回已完成第一版；尚未由 CoderAgent 自动生成 proposal，也尚未接入回滚和验证失败返工。
...
5. patch 应用后生成 artifact。（已部分完成：写入任务 `patchApplied` / `patchVerification` artifact）
...
- patch 应用后自动运行推荐验证命令。（已部分完成：`apply_approved_patch` 首次应用成功后会运行项目推荐 allowlist 命令并写入命令审计）
